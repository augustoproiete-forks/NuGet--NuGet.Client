    ...
    try {
        ...
        $tests | %{
            ...
            $testCases | %{
				...
				$startTime = Get-Date			
				Write-Host $startTime.ToString() " : IsBindingRedirect before test : " $name " is " $IsSkippedBefore
				...
				$startTime = Get-Date			
				Write-Host $startTime.ToString() " : IsBindingRedirect before test : " $name " is " $IsSkippedAfter | timestamp
